...
const Ctx = createContext<{ lang: Lang; t: (k: StringKey) => string; toggle: () => void }>({
  lang: "en",
  t: (k) => strings.en[k],
  toggle: () => {},
});
...
export function I18nProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const stored = window.localStorage.getItem("tantuve-lang");
  ...
  const toggle = () => {
    setLang((prev) => {
      ...
      window.localStorage.setItem("tantuve-lang", next);
      return next;
    });
  };

  return (
    <Ctx.Provider value={{ lang, t: (k) => strings[lang][k], toggle }}>{children}</Ctx.Provider>
  );
}
