...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      { title: "Sign in — Tantuve" },
      {
        name: "description",
        content:
          "Sign in to Tantuve as a weaver, GI authority or retailer to manage handloom provenance records.",
      },
      { property: "og:title", content: "Sign in — Tantuve" },
      { property: "og:description", content: "Access your Tantuve traceability dashboard." },
...
function LoginPage() {
  ...
  useEffect(() => {
    if (session && role) void navigate({ to: roleHome[role] });
      ...
      <div className="mx-auto grid max-w-5xl gap-10 px-4 py-16 sm:px-6 md:grid-cols-2">
        <div>
          ...
          <p className="mt-4 text-sm text-muted-foreground">
            New weaver?{" "}
            <Link to="/apply" className="text-madder hover:underline">
