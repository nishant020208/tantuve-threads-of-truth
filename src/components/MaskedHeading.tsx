...
const MaskedHeading = ({
  ...
  settingsRef.current = { fillScale, parallax, drift, brightness, saturation, grayscale: grayscale ? 1 : 0, textScale };
  ...
  const place = useCallback(() => {
    ...
    const maxX = Math.max(0, ((Number(s.fillScale) - 1) / 2) * W);
    const maxY = Math.max(0, ((Number(s.fillScale) - 1) / 2) * H);
    media.style.transform = `translate3d(${clamp(off.x, -maxX, maxX).toFixed(2)}px, ${clamp(off.y, -maxY, maxY).toFixed(2)}px, 0) scale(${Number(s.fillScale)})`;
    media.style.filter = `brightness(${Number(s.brightness)}) saturate(${Number(s.saturation)})${s.grayscale ? " grayscale(1)" : ""}`;
  }, []);
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      const s = settingsRef.current;
      const off = offsetRef.current;
      const dx = Math.sin(clock * 0.21) * Number(s.drift);
      const dy = Math.cos(clock * 0.17) * Number(s.drift) * 0.6;
    ...
    const onMove = (e: PointerEvent) => {
      const s = settingsRef.current;
      if (Number(s.parallax) <= 0) return;
      const r = root.getBoundingClientRect();
      const nx = ((e.clientX - r.left) / (r.width || 1)) * 2 - 1;
      const ny = ((e.clientY - r.top) / (r.height || 1)) * 2 - 1;
      offsetRef.current.tx = clamp(nx, -1, 1) * -Number(s.parallax);
      offsetRef.current.ty = clamp(ny, -1, 1) * -Number(s.parallax);
    };
