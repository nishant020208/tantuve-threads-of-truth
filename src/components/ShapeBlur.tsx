...
void main() {
    ...
    vec3 color = u_tintColor;
    float alpha = sdf;
    gl_FragColor = vec4(color.rgb, alpha);
}
`;

function getThemeTintColor(theme: ThemeMode): [number, number, number] {
  switch (theme) {
    case "aesthetic":
      return [0.83, 0.63, 0.09]; // #D4A017 gold
    case "white":
      return [0.55, 0.12, 0.25]; // #8B1E3F madder
    case "black":
      return [0.94, 0.78, 0.25]; // #F0C840 bright gold
  }
}
...
const ShapeBlur: FC<ShapeBlurProps> = ({
  ...
  const tintRef = useRef(getThemeTintColor(theme));

  // Update tint when theme changes
  useEffect(() => {
    tintRef.current = getThemeTintColor(theme);
    const mat = materialRef.current;
    if (mat) {
      const t = tintRef.current;
      mat.uniforms.u_tintColor.value.set(t[0], t[1], t[2]);
    }
  }, [theme]);
  ...
  useEffect(() => {
    ...
    const t = tintRef.current;
    const vTint = new THREE.Vector3(t[0], t[1], t[2]);
