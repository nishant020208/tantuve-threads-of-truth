...
export async function POST(
  ...
  const nextSeq = existing && existing.length > 0 ? existing[0].sequence_no + 1 : 1;
  ...
  if (existing && existing.length > 0) {
    const { data: prev } = await client
      ...
      .single();
    if (prev?.created_at) {
      const elapsed = (new Date(now).getTime() - new Date(prev.created_at).getTime()) / 1000;
  ...
  const prevHash = nextSeq > 1 && existing?.length
    ? (await client.from("ledger_entries").select("entry_hash").eq("product_id", productId).order("sequence_no", { ascending: false }).limit(1).single()).data?.entry_hash || ""
    : "";
  const payload = JSON.stringify({ step_name: body.step_name, step_data: body.step_data, actor: body.actor || user.userId, sequence_no: nextSeq, prev_hash: prevHash, created_at: now });
